...
def all_partitions_of(a_list):
    ...
    if len(a_list) < 2 or len(a_list) == 0:
        returned_partitions = [[a_list]]
        return returned_partitions
    else:
        i, returned_partitions = 1, [[[a_list[0]]]]
        while i < len(a_list):
            returned_partitions = extend_partitions(returned_partitions, a_list[i])
            #print("i = " + str(i))
